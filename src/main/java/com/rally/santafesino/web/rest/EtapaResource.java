package com.rally.santafesino.web.rest;

import com.codahale.metrics.annotation.Timed;
import com.rally.santafesino.service.EtapaService;
import com.rally.santafesino.web.rest.errors.BadRequestAlertException;
import com.rally.santafesino.web.rest.util.HeaderUtil;
import com.rally.santafesino.web.rest.util.PaginationUtil;
import com.rally.santafesino.service.dto.EtapaDTO;
import io.github.jhipster.web.util.ResponseUtil;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import org.springframework.http.HttpHeaders;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import java.net.URI;
import java.net.URISyntaxException;

import java.util.List;
import java.util.Optional;

/**
 * REST controller for managing Etapa.
 */
@RestController
@RequestMapping("/api")
public class EtapaResource {

    private final Logger log = LoggerFactory.getLogger(EtapaResource.class);

    private static final String ENTITY_NAME = "etapa";

    private final EtapaService etapaService;

    public EtapaResource(EtapaService etapaService) {
        this.etapaService = etapaService;
    }

    /**
     * POST  /etapas : Create a new etapa.
     *
     * @param etapaDTO the etapaDTO to create
     * @return the ResponseEntity with status 201 (Created) and with body the new etapaDTO, or with status 400 (Bad Request) if the etapa has already an ID
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PostMapping("/etapas")
    @Timed
    public ResponseEntity<EtapaDTO> createEtapa(@RequestBody EtapaDTO etapaDTO) throws URISyntaxException {
        log.debug("REST request to save Etapa : {}", etapaDTO);
        if (etapaDTO.getId() != null) {
            throw new BadRequestAlertException("A new etapa cannot already have an ID", ENTITY_NAME, "idexists");
        }
        EtapaDTO result = etapaService.save(etapaDTO);
        return ResponseEntity.created(new URI("/api/etapas/" + result.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(ENTITY_NAME, result.getId().toString()))
            .body(result);
    }

    /**
     * PUT  /etapas : Updates an existing etapa.
     *
     * @param etapaDTO the etapaDTO to update
     * @return the ResponseEntity with status 200 (OK) and with body the updated etapaDTO,
     * or with status 400 (Bad Request) if the etapaDTO is not valid,
     * or with status 500 (Internal Server Error) if the etapaDTO couldn't be updated
     * @throws URISyntaxException if the Location URI syntax is incorrect
     */
    @PutMapping("/etapas")
    @Timed
    public ResponseEntity<EtapaDTO> updateEtapa(@RequestBody EtapaDTO etapaDTO) throws URISyntaxException {
        log.debug("REST request to update Etapa : {}", etapaDTO);
        if (etapaDTO.getId() == null) {
            return createEtapa(etapaDTO);
        }
        EtapaDTO result = etapaService.save(etapaDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(ENTITY_NAME, etapaDTO.getId().toString()))
            .body(result);
    }

    /**
     * GET  /etapas : get all the etapas.
     *
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of etapas in body
     */
    @GetMapping("/etapas")
    @Timed
    public ResponseEntity<List<EtapaDTO>> getAllEtapas(Pageable pageable) {
        log.debug("REST request to get a page of Etapas");
        Page<EtapaDTO> page = etapaService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/etapas");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }

    /**
     * GET  /etapas/:id : get the "id" etapa.
     *
     * @param id the id of the etapaDTO to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the etapaDTO, or with status 404 (Not Found)
     */
    @GetMapping("/etapas/{id}")
    @Timed
    public ResponseEntity<EtapaDTO> getEtapa(@PathVariable Long id) {
        log.debug("REST request to get Etapa : {}", id);
        EtapaDTO etapaDTO = etapaService.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(etapaDTO));
    }

    /**
     * DELETE  /etapas/:id : delete the "id" etapa.
     *
     * @param id the id of the etapaDTO to delete
     * @return the ResponseEntity with status 200 (OK)
     */
    @DeleteMapping("/etapas/{id}")
    @Timed
    public ResponseEntity<Void> deleteEtapa(@PathVariable Long id) {
        log.debug("REST request to delete Etapa : {}", id);
        etapaService.delete(id);
        return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, id.toString())).build();
    }
}
